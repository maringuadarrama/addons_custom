...
export class ThreedArchParser {
    /**
     * @param {HTMLElement} arch
     */
    parse(arch) {
        const noAreaLoaded = arch.getAttribute("no_area_loaded");
        const noItem = arch.getAttribute("no_item");
        let autoRefreshAttrNode = arch.getAttribute("auto_refresh");
        ...
        /** @type {CameraConfig} */
        const camera = {};
        /** @type {Partial<LegendFieldsMapping>} */
        const legendFieldsMapping = {};
        /** @type {Partial<AreaFieldsMapping>} */
        const areaFieldsMapping = {};
        /** @type {Partial<ItemFieldsMapping>} */
        ...
        visitXML(arch, (node) => {
            ...
            switch (node.tagName) {
                case "camera": {
                    if (node.getAttribute("x") && !isNaN(parseInt(node.getAttribute("x")))) {
                        camera.camx = parseInt(node.getAttribute("x"));
                    }
                    if (node.getAttribute("y") && !isNaN(parseInt(node.getAttribute("y")))) {
                        camera.camy = parseInt(node.getAttribute("y"));
                    }
                    if (node.getAttribute("z") && !isNaN(parseInt(node.getAttribute("z")))) {
                        camera.camz = parseInt(node.getAttribute("z"));
                    }
                    if (node.getAttribute("fov") && !isNaN(parseInt(node.getAttribute("fov")))) {
                        ...
                        switch (node.parentNode.nodeName) {
                            ...
                            case "objects": {
                                if (node.getAttribute("type") === "gltf") {
                                    itemFieldsMapping.gltfField = node.getAttribute("name");
                                    break;
                                }
                                if (node.getAttribute("type") === "scale_factor") {
                                    itemFieldsMapping.scaleFactorField = node.getAttribute("name");
                                    break;
                                }
                                if (node.getAttribute("type") === "name") {
                                    itemFieldsMapping.nameField = node.getAttribute("name");
                                    break;
                                }
                                if (node.getAttribute("type") === "geometry") {
                                    itemFieldsMapping.geometryField = node.getAttribute("name");
                                    break;
                                }
                                if (node.getAttribute("type") === "color") {
                                    itemFieldsMapping.colorField = node.getAttribute("name");
                                    break;
                                }
                                if (node.getAttribute("type") === "opacity") {
                                    itemFieldsMapping.opacityField = node.getAttribute("name");
                                    break;
                                }
                                if (node.getAttribute("type") === "size") {
                                    itemFieldsMapping[`size${node.getAttribute("axis")}Field`] =
                                        node.getAttribute("name");
                                    break;
                                }
                                if (node.getAttribute("type") === "position") {
                                    itemFieldsMapping[`position${node.getAttribute("axis")}Field`] =
                                        node.getAttribute("name");
                                    break;
                                }
                                if (node.getAttribute("type") === "rotation") {
                                    itemFieldsMapping[`rotation${node.getAttribute("axis")}Field`] =
                                        node.getAttribute("name");
                                    break;
                                }
                            }
