...
export function useAbsoluteThreedPopUp(popupRef, containerRef) {
     ...
     */
    function _popUpPositionRenderer(popupRef, showPopUp, mousePositionX, mousePositionY, sceneContainerRef) {
        if (showPopUp) {
            ...
            if (parseInt($popup.css("right"), 10) < 0) {
                $popup.css("left", mousePositionX - $container.offset().left - $popup.width() - 30);
            }
            if (parseInt($popup.css("bottom"), 10) < 0) {
                $popup.css("top", mousePositionY - $container.offset().top - $popup.height() - 30);
            }
            if (parseInt($popup.css("bottom"), 10) < 0) {
                $popup.css("top", mousePositionY - $container.offset().top - $popup.height() - 30);
            }
