...
export function useThreedScene(containerRef, initConfig) {
    ...
    function init3d() {
        // camera
        camera = new THREE.PerspectiveCamera(config.camFov, window.innerWidth / window.innerHeight, 1, 3000);
        camera.position.fromArray(realTo3dvSizes(config.camPosX_real, -config.camPosY_real, config.camPosZ_real));
        // controls
        ...
        target.position.fromArray(realTo3dvSizes(config.targetX_real, -config.targetY_real, config.targetZ_real));
        target.name = "target";
    ...
    function addObject(item3d, type, defaultScale) {
        const size = realTo3dvSizes(item3d.sizex, item3d.sizey, item3d.sizez, defaultScale);
    ...
    function renderGround() {
        ...
        var groundGeometry = new THREE.BoxGeometry(config.groundX_3dv, 2, config.groundZ_3dv);
        var groundMaterial = new THREE.MeshStandardMaterial({
            map: texture,
            flatShading: true,
        });
        var ground = new THREE.Mesh(groundGeometry, groundMaterial);
        ground.position.fromArray(realTo3dvCoords(0, 0, 0, config.groundX_real, config.groundY_real, 0)).y = -1;
        ...
        underground.position.fromArray(realTo3dvCoords(0, 0, 0, config.groundX_real, config.groundY_real, 0)).y = -3;
        ground.name = "ground";
        underground.name = "underground";
        scene.add(ground);
        scene.add(underground);

        var grid = new THREE.GridHelper(2000, (2 * config.groundX_real) / config.groundX_3dv);
    ...
    function findObjectOnClick(event, filterCallback) {
        mouse.x =
            ((event.clientX - $(sceneContainerRef.el).offset().left - 10) / renderer.domElement.clientWidth) * 2 - 1;
        mouse.y =
            -((event.clientY - $(sceneContainerRef.el).offset().top - 10) / renderer.domElement.clientHeight) * 2 + 1;
        raycaster.setFromCamera(mouse, camera);

        var intersects = raycaster.intersectObjects(areaObjects.meshes.filter(filterCallback));
