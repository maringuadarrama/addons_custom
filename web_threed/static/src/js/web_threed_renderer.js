...
export class ThreedRenderer extends Component {
    setup() {
        ...
        this.threeDScene = useThreedScene("scene-container", this.derive3DSceneConfigFromProps(this.props));
        this.state = useState({
        ...
        useEffect(
            () => {
                this.threeDScene.setConfig(this.derive3DSceneConfigFromProps(this.props));
    ...
    derive3DSceneConfigFromProps(props) {
        ...
        const area = props.groundAreas.find(
            (/** @type {import("./web_threed_model").GroundArea} */ a) => a.id === props.selectedGroundAreaId
        );
        ...
        config.groundZ_3dv = Math.round((config.groundY_real / config.groundX_real) * config.groundX_3dv);
        config.heightY_3dv = Math.round((config.heightZ_real / config.groundX_real) * config.groundX_3dv);
     ...
     */
    addWireframe(item3d) {
        this.threeDScene.addObject(item3d, "wireframe", this.getScaleForArea(item3d.areaId));
    }
    ...
    getScaleForArea(areaId) {
        const area = this.props.groundAreas.find(
            (/** @type {import("./web_threed_model").GroundArea} */ a) => a.id === areaId
        );
    ...
    showWireframesForAllItems3d() {
        this.props.all3dItems.forEach((/** @type {import("./web_threed_model").Item3d} */ item3d) => {
    ...
    showMeshesForSelectedItems3d(selected3dItems) {
        ...
        selected3dItems.forEach((/** @type {import("./web_threed_model").Item3d} */ item3d) => {
            this.addMesh(item3d);
    ...
    manageGLTFObjectsVisibility() {
        ...
        for (let key in this.threeDScene.areaObjects.gltfObjects) {
            this.threeDScene.areaObjects.gltfObjects[key].model.visible =
                this.threeDScene.areaObjects.gltfObjects[key].area == this.state.currentArea.id;
            this.threeDScene.areaObjects.gltfObjects[key].light.intensity =
                this.threeDScene.areaObjects.gltfObjects[key].area == this.state.currentArea.id ? 7 : 0;
        }
    }
    ...
    showGLTFModel(item3d) {
        ...
        this.threeDScene.GLTFLoader.load(item3d.gltf_load_url, (gltf) => {
            ...
            if (mod.children && mod.children[0]) {
                mod.children[0].scale.set(item3d.scaleFactor, item3d.scaleFactor, item3d.scaleFactor);
            } else {
                mod.scale.set(item3d.scaleFactor, item3d.scaleFactor, item3d.scaleFactor);
            }
            ...
            plight.position.set(mod.position.x + 20, this.threeDScene.config.heightY_3dv, mod.position.z + 15);
    ...
    async onContainerDoubleClick(event) {
        ...
        if (item3d && item3d instanceof THREE.Mesh) {
            ...
            let wireframe = this.threeDScene.scene.getObjectByName("wireframe" + item3d.userData.item3d.id);
            ...
            const info = await this.props.loadInfoForObjectId(item3d.userData.item3d.id);
        ...
        if (this.coordinatesDisplay.visible && !this.itemInfoPopup.state.visible) {
            ...
            if (item3d) {
                ...
                this.barcodeLabel.el.style.left =
                    event.clientX - getOffset(this.sceneDOMContainer.el).left + 20 + "px";
                this.barcodeLabel.el.style.top = event.clientY - getOffset(this.sceneDOMContainer.el).top - 5 + "px";
    ...
    closeItemInfoPopup(e) {
        e.preventDefault();
        var item3d = this.threeDScene.scene.getObjectByName("mesh" + this.itemInfoPopup.state.currentId);
        if (item3d && item3d instanceof THREE.Mesh) {
            item3d.material.color = new THREE.Color(item3d.userData.item3d.color);
            var wireframe = this.threeDScene.scene.getObjectByName("wireframe" + item3d.userData.item3d.id);
            if (wireframe && wireframe instanceof THREE.LineSegments) {
