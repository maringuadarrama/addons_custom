...
export class ThreedModel {
    ...
    constructor(orm, resModel, fields, archInfo, domain, context, rpc) {
        ...
        /** @type {ArchInfo} */
        const {readFields, itemFieldsMapping, areaFieldsMapping, legendFieldsMapping, legendModel} = archInfo;
        /** @type {string} */
    ...
    mapRecordToItem3d(record) {
        const item3d = {
            ...
            areaId: record[this.itemFieldsMapping.areaField],
    ...
    async _getAreas(areaIds) {
        const {records} = await this.keepLast.add(
            this.orm.webSearchRead(this.areaModel, [["id", "in", areaIds]], {
                specification: {
    ...
    async _getSelectedItems() {
        const {records} = await this.keepLast.add(
            this.orm.webSearchRead(this.resModel, this.domain.concat(this.withSizeDomain), {
                specification: this.readFields.reduce((acc, curr) => {
                    return {
                        ...acc,
                        [curr]: {},
                    };
        ...
        /** @type {Item3d[]} */
        const selected3dItems = records.map((/** @type {{ [x: string]: any; id: number; }} */ record) =>
    ...
    async _getAllItems(groundAreaIds) {
        /** @type any[] */
        const allItemsForAreaDomain = [[this.itemFieldsMapping.areaField, "in", groundAreaIds]];
        const {records} = await this.keepLast.add(
            this.orm.webSearchRead(this.resModel, allItemsForAreaDomain.concat(this.withSizeDomain), {
                specification: this.readFields.reduce((acc, curr) => {
                    return {
                        ...acc,
                        [curr]: {},
        ...
        /** @type {Item3d[]} */
        const all3dItems = records.map((/** @type {{ [x: string]: any; id: number; }} */ record) =>
            this.mapRecordToItem3d(record)
        );
        return all3dItems;
    }
    ...
    async load() {
        ...
        /** @type {number[]} */
        const groundAreaIds = [...new Set(this.selected3dItems.map((/** @type {Item3d} */ item) => item.areaId))];
    ...
    async _loadLegend() {
        ...
        if (this.legendModel && this.legendModel !== "") {
            const {records} = await this.keepLast.add(
                this.orm.webSearchRead(this.legendModel, [], {
                    specification: {
            ...
            /** @type {Legend[]} */
            const results = records.map((/** @type {{ [x: string]: any; }} */ record) => {
                /** @type {Legend} */
                return {
