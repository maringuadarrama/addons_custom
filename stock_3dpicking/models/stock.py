...
class StockPicking(models.Model):
    ...
    def action_locate(self):
        ...
        return {
            ...
            "context": dict(
                self.env.context,
                request_type="picking_locations",
                force_color="#875A7B",
                force_opacity=700,
                force_legend_items=[
                    {"name": "picking", "color": "#875A7B", "opacity": 1},
                ],
            ),
        }
...
class StockPickingBatch(models.Model):
    ...
    def action_locate_batch(self):
        ...
        batch_location_ids = (
            self.env["stock.quant"].search([("product_id", "in", batch_product_ids)]).mapped("location_id").ids
        )
        return {
            "name": _("Batch"),
            "type": "ir.actions.act_window",
            ...
            "context": dict(
                self.env.context,
                request_type="batch_picking_locations",
                force_color="#875A7B",
                force_opacity=700,
                force_legend_items=[
                    {"name": "batch picking", "color": "#875A7B", "opacity": 1},
                ],
            ),
        }
...
class StockWarehouse(models.Model):
    ...
    def action_locate_empty_locations(self):
        all_locations_ids = self.env["stock.location"].search([("warehouse_id", "=", self.id)]).ids
        full_locations_ids = (
            self.env["stock.quant"]
            .search([("location_id.warehouse_id", "=", self.id), ("quantity", ">", 0)])
            .mapped("location_id")
            .ids
        )
        ...
        return {
            ...
            "context": dict(
                self.env.context,
                request_type="empty_locations",
                force_color="#FF0000",
                force_opacity=1000,
                force_legend_items=[
                    {"name": "empty", "color": "#FF0000", "opacity": 1},
                ],
            ),
        }
    ...
    def action_locate_not_empty_locations(self):
        ...
        return {
            ...
            "context": dict(
                self.env.context,
                request_type="not_empty_locations",
                force_color="#875A7B",
                force_opacity=700,
                force_legend_items=[
                    {"name": "not empty", "color": "#875A7B", "opacity": 1},
                ],
            ),
        }
