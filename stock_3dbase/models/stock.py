    ...
    def _compute_3d_attributes(self):
        for location in self:
            if self.env.context.get("force_color", False):
                ...
                location.color = _colors[location.tag_ids[0].color] if location.tag_ids else "#8F8F8F"
            if self.env.context.get("force_opacity", False):
                location.opacity = int(self.env.context.get("force_opacity"))
            else:
                location.opacity = location.tag_ids[0].opacity if location.tag_ids else 500
            location.shape_file = location.shape_id.shape_file if location.shape_id else False
